class Notification < ApplicationRecord
  ...
  private
  def set_message_key
    self.message_key = "notifications.#{self.notification_type}"
  end
  ...
  def set_message_params
    ...
    when "strawberry_badge_award"
      self.message_params =  { username: self.user.username }
    ...
    when "level_up"
      self.message_params =  { username: self.user.username, level: self.user.level }
      "#{self.user.username} reached level #{self.user.level}!"
    when "experience_point_award"
      self.message_params =  { amount: self.source.amount, award_type: self.source.award_type }
      "You got #{self.source.amount} #{self.source.award_type} points!"
    when "fruit_ticket_gift"
      self.message_params =  { username: self.user.username, amount: self.user.amount }
      "#{self.source.from_user.username} sent you Ƒ#{self.source.amount} fruit tickets!"
    when "supporter_fruit_ticket_stipend"
      self.message_params = { amount: self.user.amount }
    ...
    when "glorp_lottery_winner"
      self.message_params =  { username: self.user.username, amount: self.source.amount, award_type: self.source.award_type }
      ":#{self.source.award_type.split("py").first}:!!! #{self.user.username} got #{self.source.amount} #{self.source.award_type} points!"
    when "show_comment"
      self.message_params =  { title: self.source.title }
      "#{self.source.title} has a new comment!"
    when "patreon_sub"
      self.message_params =  { name: self.source.name }
    ...
    when "avatar_update"
      self.message_params =  { username: self.user.username, image_url: self.source.image.url(:thumb) }
    ...
    when "shrimpo_entry"
      self.message_params =  { title: self.source.title, entries_count: self.source.shrimpo.shrimpo_entries.count, emoji: self.source.shrimpo.emoji }
