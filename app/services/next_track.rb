class NextTrack
  ...
  #
  # side effect: sets the current show playing in redis , so we can compare in the schedule_monitor for skipping
  ...
  def self.perform radio
    ...
    { cue_out: cue_out.to_i.to_s, cue_in: 0.to_s, fade_out: 0.to_s, fade_in: 0.to_s, track: track.s3_filepath }
  end
end
