...
describe ScheduleMonitor do
  ...
  let(:playlist) { FactoryGirl.create :playlist, radio: radio }
  let(:show){ FactoryGirl.create :scheduled_show }
  ...
  it "issues a skip if a show is scheduled and is not currently playing" do
    scheduled_show = FactoryGirl.create :scheduled_show, playlist: playlist, radio: radio,
